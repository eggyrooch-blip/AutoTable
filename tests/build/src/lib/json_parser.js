...
exports.parseInputToSpecs = parseInputToSpecs;
...
function unwrapCommonContainers(obj) {
    if (obj == null)
        return obj;
...
function topLevelObjectHasObjectArrays(x) {
...
function sampleRecords(arr, max = 200) {
...
function parseInputToSpecs(input) {
    const warnings = [];
    ...
    if (unwrapped && typeof unwrapped === 'object') {
        ...
        if (keys.length > 0) {
            const tables = keys.map(k => ({
                name: k,
                fields: [],
                records: sampleRecords(unwrapped[k]),
            }));
