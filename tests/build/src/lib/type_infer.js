...
exports.inferFieldsFromRecords = inferFieldsFromRecords;
...
function isLikelyProgress(num) {
    return num >= 0 && num <= 100;
}
function isLikelyRating(num) {
    return num >= 0 && num <= 5;
}
function chooseStringType(s) {
    if (URL_REGEX.test(s))
        return 'Url';
...
function inferPrimitiveType(value) {
    ...
    if (typeof value === 'number') {
        if (isLikelyProgress(value))
            return 'Progress';
        if (isLikelyRating(value))
            return 'Rating';
        return 'Number';
    }
    // 布尔类型
    ...
    if (typeof value === 'object') {
        ...
        if ('token' in value)
            return 'Attachment';
        if ('amount' in value || 'currency' in value || 'currencyCode' in value)
            return 'Currency';
    }
    return 'Text';
}
function aggregateOptions(values) {
    const set = new Set();
...
function getSuggestedTypes(value) {
        ...
        for (const key of Object.keys(rec)) {
            ...
            if (Array.isArray(val)) {
                ...
                if (options && options.length > 0) {
                    ...
                    const merged = new Set([...((prev === null || prev === void 0 ? void 0 : prev.options) || []), ...options]);
