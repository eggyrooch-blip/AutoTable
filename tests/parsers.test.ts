import { parseYamlData, detectFormat, parseLogData, parseDelimitedText } from '../src/lib/parsers';
...
function testYamlParsing() {
  const result = parseYamlData(yamlSample);
  ...
  const nested = parseYamlData(nestedYaml) as Record<string, any>;
...
function testFormatDetection() {
  assertEqual(detectFormat(yamlSample), 'yaml');
  assertEqual(detectFormat(xmlSample), 'xml');
  assertEqual(detectFormat(logSample), 'log');
}

function testLogParsing() {
  const records = parseLogData(logSample);
...
function testTsvParsing() {
  const rows = parseDelimitedText(tsvSample, '\t');
